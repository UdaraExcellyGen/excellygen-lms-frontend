...
const EnrollmentChart: React.FC = () => {
  ...
  if (error && !categories.length) return <div className="p-6 text-red-500">Error: {error}</div>;

  return (
    <div className="p-6">
      <h2 className="text-xl font-['Unbounded'] mb-6 text-[#1B0A3F]">Enrollment Analytics</h2>
      ...
      <div className="mb-6">
        ...
        <Select
          ...
          styles={{
            ...
            option: (base, state) => ({
              ...base,
              backgroundColor: state.isSelected ? '#BF4BF6' : state.isFocused ? 'rgba(191, 75, 246, 0.1)' : base.backgroundColor,
              color: state.isSelected ? 'white' : base.color,
              '&:active': {
                backgroundColor: 'rgba(191, 75, 246, 0.2)'
              }
            }),
            input: (base) => ({
              ...base,
      ...
      {showChart ? (
        loading ? (
          <div className="text-center py-10">
            <svg className="animate-spin h-8 w-8 text-[#BF4BF6] mx-auto" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
              <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle>
              <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
            </svg>
            <p className="mt-2 text-[#1B0A3F]">Loading enrollment data...</p>
          </div>
        ) : error ? (
          <div className="text-center py-10 text-red-500">{error}</div>
        ) : data.length === 0 ? (
          <div className="text-center py-10 text-[#1B0A3F]">No enrollment data available for this category</div>
        ) : (
          <div className="h-64">
            <ResponsiveContainer width="100%" height="100%">
              ...
              >
                ...
                <XAxis 
                  dataKey="name" 
                  tick={{ fontSize: 14, fill: "#1B0A3F" }}
                  interval={0}
                  tickMargin={12}
                  label={{ value: "Courses", position: "bottom", offset: 5, fill: "#1B0A3F" }}
                />
                <YAxis 
                  allowDecimals={false}
                  domain={[0, 'dataMax']}
                    ...
                    if (active && payload && payload.length) {
                      return (
                        <div className="bg-white p-2 rounded shadow-lg border border-[#BF4BF6]/20">
                          <p className="text-sm font-semibold text-[#1B0A3F]">{payload[0].payload.fullName}</p>
                          <p className="text-sm text-[#BF4BF6]">Learners: {payload[0].value}</p>
                        </div>
                      );
                    }
                    return null;
        ...
        <div className="text-center py-10 text-gray-500">
          <svg xmlns="http://www.w3.org/2000/svg" className="h-16 w-16 mx-auto text-[#BF4BF6]/30 mb-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          <p className="text-[#1B0A3F]">Please select a course category to view enrollment analytics</p>
          <p className="mt-2 text-sm text-[#BF4BF6]">
            {categories.length > 0 ? 
