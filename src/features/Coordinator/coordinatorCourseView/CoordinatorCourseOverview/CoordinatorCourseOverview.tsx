...
const CoordinatorCourseOverview: React.FC = () => {
    ...
    const handleDeactivateCourse = async () => {
        if (!courseId || !window.confirm("Are you sure you want to deactivate this course? It will be hidden from the catalog and new enrollments will be disabled.")) return;
        setIsSaving(true);
        ...
        try {
            ...
            setLastRefresh(Date.now()); // Refresh data
        } catch (error) {
    ...
    const handleReactivateCourse = async () => {
        if (!courseId || !window.confirm("Are you sure you want to reactivate this course? It will become visible in the catalog again.")) return;
        setIsSaving(true);
    ...
    const handlePublishCourse = async () => {
        if (!courseId || !window.confirm("Are you sure you want to publish this course?")) return;
        setIsSaving(true);
    ...
    const handleEditCourseInputChange = useCallback((e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>) => {
        ...
        setEditCourseDetails(prev => {
            if (!prev) return null;
            if (name === "thumbnail") return { ...prev, thumbnail: (e.target as HTMLInputElement).files?.[0] || null };
            return { ...prev, [name]: name === "estimatedTime" ? parseInt(value, 10) || 0 : value };
        });
    }, []);
    ...
    const handleSaveCourse = async () => {
        ...
        try {
            ...
            // Instead of refetching, just update the state optimistically
            setCourseData(prev => prev ? { ...prev, ...coursePayload } : null);
            setIsEditMode(false);
    ...
    const handleAddSubtopic = async () => {
        ...
        try {
            const newLessonDto = await addLesson({ courseId, lessonName: "New Subtopic", lessonPoints: 1 });
    ...
    const handleRemoveSubtopic = async (lessonId: number, lessonName: string) => {
        if (!window.confirm(`Are you sure you want to delete "${lessonName}"? This is permanent.`)) return;
        setIsSaving(true);
    ...
    const askDeleteConfirmation = useCallback((documentId: number, lessonId: number, documentName: string) => { setMaterialToDelete({ documentId, lessonId, name: documentName }); setDeleteDialogOpen(true); }, []);
    ...
    const handleConfirmDelete = async () => {
        ...
        try {
            ...
            setCourseData(prev => prev ? {
                ...prev,
                lessons: prev.lessons.map(l => l.id === materialToDelete.lessonId ? { ...l, documents: l.documents.filter(d => d.id !== materialToDelete.documentId) } : l)
            } : null);
            toast.success(`"${materialToDelete.name}" deleted.`, { id: deleteToastId });
        } catch (error) {
    ...
    const handleRemoveQuiz = useCallback(async (lessonId: number) => {
        if (!courseId || !window.confirm(`Are you sure you want to delete this quiz?`)) return;
        setIsSaving(true);
        ...
        try {
            ...
            if (quizzes.length > 0) {
                ...
                setCourseData(prev => prev ? {
                    ...prev,
                    lessons: prev.lessons.map(l => l.id === lessonId ? { ...l, quizzes: [] } : l)
                } : null);
                toast.success("Quiz deleted.", { id: deleteQuizToast });
            } else {
                toast.error("No quiz found.", { id: deleteQuizToast });
            }
        } catch (error) {
            toast.error("Failed to delete quiz.", { id: deleteQuizToast });
        } finally {
            setIsSaving(false);
        }
    }, [courseId]);
    ...
    return (
        <div className="min-h-screen bg-gradient-to-b from-[#52007C] to-[#34137C] font-nunito">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
                ...
                <div className="bg-[#1B0A3F]/60 backdrop-blur-md rounded-2xl p-6 shadow-lg">
                        ...
                        <div className="md:col-span-1">
                            {isEditMode ? (
                                <div className="relative">
                                    <img src={courseData.thumbnailUrl || undefined} alt={courseData.title} className="w-full h-48 object-cover rounded-xl shadow-lg opacity-80" />
                                ...
                                <img src={courseData.thumbnailUrl || undefined} alt={courseData.title} className="w-full h-48 object-cover rounded-xl shadow-lg" />
                            )}
                        </div>
                            ...
                            {isEditMode && editCourseDetails ? ( // Check for editCourseDetails
                                <div className="space-y-4">
                                    <div>
                                        ...
                                        <input type="text" name="title" value={editCourseDetails.title} onChange={handleEditCourseInputChange} className="w-full bg-[#34137C]/50 border border-[#BF4BF6]/30 rounded-lg p-2 text-white focus:outline-none focus:border-[#BF4BF6]" />
                                    </div>
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        <div>
                                            <label className="block text-[#D68BF9] text-sm mb-1">Category</label>
                                            <select name="categoryId" value={editCourseDetails.categoryId} onChange={handleEditCourseInputChange} className="w-full bg-[#34137C]/50 border border-[#BF4BF6]/30 rounded-lg p-2 text-white focus:outline-none focus:border-[#BF4BF6]">
                                        ...
                                        <div>
                                            ...
                                            <input type="number" name="estimatedTime" value={editCourseDetails.estimatedTime} onChange={handleEditCourseInputChange} min="1" className="w-full bg-[#34137C]/50 border border-[#BF4BF6]/30 rounded-lg p-2 text-white focus:outline-none focus:border-[#BF4BF6]" />
                                        </div>
                                    </div>
                                    <div>
                                        ...
                                        <div className="bg-[#34137C]/50 border border-[#BF4BF6]/30 rounded-lg p-2 flex flex-wrap gap-2">
                                            {availableTechnologies.map(t => (
                                                <label key={t.id} className="inline-flex items-center">
                                                    ...
                                                    <span className="ml-2 text-white text-sm">{t.name}</span>
                                                </label>
                                            ))}
                                        </div>
                                    </div>
                                    <div>
                                        ...
                                        <textarea name="description" value={editCourseDetails.description || ''} onChange={handleEditCourseInputChange} rows={3} className="w-full bg-[#34137C]/50 border border-[#BF4BF6]/30 rounded-lg p-2 text-white focus:outline-none focus:border-[#BF4BF6]" />
                                    </div>
                                </div>
                            ) : (
                                <>
                                    <div className="flex justify-between items-start mb-3">
                                        ...
                                        <div className="flex gap-2">
                                            ...
                                            {isInactive ? (
                                                <button onClick={handleReactivateCourse} disabled={isSaving} className="bg-green-500 hover:bg-green-600 text-white px-4 py-2 rounded-lg flex items-center gap-2 transition-colors">
                                                    <RotateCcw size={16} /> Reactivate
                                                </button>
                                            ) : (
                                                <>
                                                    {courseData.status === 'Draft' && (
                                                        <button onClick={handlePublishCourse} disabled={isSaving} className="bg-blue-500 hover:bg-blue-600 text-white px-4 py-2 rounded-lg flex items-center gap-2 transition-colors">
                                                            Publish
                                                        </button>
                                                    )}
                                                    <button onClick={handleToggleEditMode} disabled={isSaving || isInactive} className="bg-[#BF4BF6] hover:bg-[#D68BF9] text-white px-4 py-2 rounded-lg flex items-center gap-1 transition-colors shadow-lg disabled:opacity-50 disabled:cursor-not-allowed">
                                                        <Edit size={16} /> Edit
                                                    </button>
                                                    {courseData.status === 'Published' && (
                                                        <button onClick={handleDeactivateCourse} disabled={isSaving} className="bg-red-500 hover:bg-red-600 text-white px-4 py-2 rounded-lg flex items-center gap-1 transition-colors">
                                    ...
                                    <div className="flex flex-wrap gap-2 mb-4">
                                        ...
                                        {courseData.technologies.map(t => <span key={t.id} className="bg-[#34137C] text-white px-3 py-1 rounded-full text-sm">{t.name}</span>)}
                                    </div>
                    ...
                    {isEditMode && (
                        <div className="mt-6 flex justify-end gap-3">
                            <button onClick={handleToggleEditMode} disabled={isSaving} className="bg-gray-600 hover:bg-gray-500 text-white px-4 py-2 rounded-lg transition-colors">Cancel</button>
                ...
                <div className="bg-[#1B0A3F]/60 backdrop-blur-md rounded-2xl p-6 shadow-lg">
                    <div className="flex justify-between items-center mb-6">
                        <div>
                            ...
                            <p className="text-gray-400 text-sm">Organize subtopics, documents, and quizzes.</p>
                        </div>
                        {/* Disable adding lessons if the course is inactive */}
                        {isEditMode && !isPublished && !isInactive && (
                            <button onClick={handleAddSubtopic} className="bg-[#BF4BF6] hover:bg-[#D68BF9] text-white font-semibold py-2 px-4 rounded-lg transition-colors text-sm flex items-center gap-1" disabled={isSaving}>
                    ...
                    {courseData.lessons.length === 0 ? (
                        <div className="bg-[#34137C]/30 rounded-xl p-8 text-center">
                            ...
                            {isEditMode && !isPublished && !isInactive && (
                                <button onClick={handleAddSubtopic} className="bg-[#BF4BF6] hover:bg-[#D68BF9] text-white py-2 px-6 rounded-lg transition-colors flex items-center gap-1">
                ...
                <ConfirmationDialog isOpen={isDeleteDialogOpen} onConfirm={handleConfirmDelete} onCancel={handleCancelDelete} message={`Are you sure you want to remove "${materialToDelete?.name || 'this material'}"?`} />
            </div>
        </div>
    );
};
