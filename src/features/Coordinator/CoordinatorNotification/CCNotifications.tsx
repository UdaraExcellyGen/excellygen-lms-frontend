...
import NotificationGroup from './Componenets/CCnotificationGroup';
import { Notification, NotificationsState } from './types/CCNotification';
import { mockNotifications } from './data/mockData';

const CCNotifications: React.FC = () => {
  const [notifications, setNotifications] = useState<NotificationsState>(mockNotifications);
  ...
  const renderNotificationMessage = (notification: Notification) => {
    ...
    if (notification.courseName) {
      message = message.replace('{courseName}', notification.courseName);
    }

    if (notification.learnerName) {
      message = message.replace('{learnerName}', notification.learnerName);
    }

    if (notification.learnerId) {
      message = message.replace('{learnerId}', notification.learnerId);
    }
      ...
      <div className="max-w-7xl mx-auto px-8 space-y-8">
        <div className="flex justify-between items-start mb-8">
          <div>
            ...
            <p className="text-[#D68BF9] mt-1">Stay updated with your responsibilities</p>
          </div>
        </div>
...
export default CCNotifications;