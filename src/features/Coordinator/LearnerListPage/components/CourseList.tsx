...
interface CourseListProps {
    ...
    toggleCourseExpansion: (courseId: number) => void; // Expect courseId
}
        ...
        <div className="space-y-8">
            {courses.map((course) => (
                <CourseAccordionItem
                    ...
                    course={course}
                    isExpanded={!!expandedCourses[course.id]} // Check expansion using ID
                    onToggle={() => toggleCourseExpansion(course.id)} // Toggle using ID
                />
            ))}
        </div>
    );
