...
// //             title: basicDetails.title || "Agile",
...
// //                     )}
// //                     <p className="text-gray-300 mb-2"><span className="font-semibold text-gray-100">Description:</span> {displayCourse.description || "Description"}</p>
// //                     <p className="text-gray-300 mb-2"><span className="font-semibold text-gray-100">Estimated Time:</span> {displayCourse.estimatedTime} 5 hours</p>
...
//         let thumbUrl: string | null = null;
//         // Priority 1: A File object in basicDetailsFromSource (user uploaded a new image)
//         if (basicDetailsFromSource.thumbnail instanceof File) {
//             thumbUrl = URL.createObjectURL(basicDetailsFromSource.thumbnail);
//         }
//         // Priority 2: An API-provided URL (backend image)
//         else if (apiThumbnailUrl) {
//             thumbUrl = apiThumbnailUrl;
//         }
//         // Else, thumbUrl remains null

//         return {
//             id: courseIdForDisplay,
//             title: basicDetailsFromSource.title, // Removed "|| Agile" fallback
...
//             lessons: lessonsFromContext,
//         };
//     }, []); // Removed contextCourseData.createdCourseId from dependencies as it's covered by courseId
...
//         // Fetch if displayCourse is not set, or context lessons are not loaded for this course, or course ID mismatches
//         if (!displayCourse || !contextCourseData.lessonsLoaded || contextCourseData.createdCourseId !== courseId) {
//             console.log(`PublishPage: Fetching full course data for ID: ${courseId}`);
//             getCourseById(courseId)
//                 .then(fetchedCourse => {
//                     const basicDetailsForDisplay: BasicCourseDetailsState = {
...
//                         categoryId: fetchedCourse.category.id,
//                         technologies: fetchedCourse.technologies.map(t => t.id),
//                         // If context has a File for this course, it's a pending change
...
//                     setLessonsState(mappedLessons); // Update context lessons
//                     setDisplayCourse(mapToDisplayData(
//                         basicDetailsForDisplay, // Use details constructed from fetchedCourse
//                         mappedLessons,
//                         fetchedCourse.id,
//                         fetchedCourse.thumbnailUrl // Pass API thumbnail URL
//                     ));
...
//                 .finally(() => setIsLoading(false));
//         } else {
//             // Use data already in context, and displayCourse is already populated
//             console.log("PublishPage: Using course data from context and existing display state.");

//             let apiThumbnailUrlForMap: string | null = null;
//             if (contextCourseData.basicDetails.thumbnail instanceof File) {
//                 // New file uploaded in context, mapToDisplayData will create blob URL
//                 apiThumbnailUrlForMap = null; 
//             } else if (displayCourse && displayCourse.thumbnailUrl) {
//                 // No new file, preserve existing displayCourse thumbnail URL
//                 apiThumbnailUrlForMap = displayCourse.thumbnailUrl;
//             }

//             setDisplayCourse(mapToDisplayData(
//                 contextCourseData.basicDetails, // Basic details from context
//                 contextCourseData.lessons,      // Lessons from context
//                 courseId,
//                 apiThumbnailUrlForMap           // Determined thumbnail URL
//             ));
            
//             const initialExpanded: Record<number, boolean> = {};
//             contextCourseData.lessons.forEach(l => initialExpanded[l.id] = true);
//             setExpandedSubtopicsUI(initialExpanded);

//             fetchQuizzesForLessons(contextCourseData.lessons);
//             setIsLoading(false);
//         }
//     // Key dependencies: courseId, context status, and lastRefresh to force re-evaluation.
//     // displayCourse itself is now part of the condition to fetch, so not a direct dependency here to avoid loops.
//     // contextCourseData.basicDetails, contextCourseData.lessons are used, so they are dependencies.
//     }, [courseId, contextCourseData.lessonsLoaded, contextCourseData.createdCourseId, contextCourseData.basicDetails, contextCourseData.lessons, navigate, setLessonsState, mapToDisplayData, lastRefresh]);


//     useEffect(() => {
//         const handleRouteChange = () => {
//             // Only refresh if not currently loading to prevent re-triggering fetch unnecessarily mid-load
//             if (!isLoading) {
//                 setLastRefresh(Date.now());
//             }
//         };
//         handleRouteChange(); // Initial call
//         // Note: listening to location.pathname can be aggressive. Consider if a more specific trigger is needed.
//     }, [location.pathname, isLoading]); // Added isLoading to dependency
...
//             await deleteDocument(materialToDelete.documentId);
//             removeDocumentFromLessonState(materialToDelete.lessonId, materialToDelete.documentId);
//             // displayCourse will update via context change -> useEffect re-run
//             toast.dismiss(loadingToastId);
//             toast.success(`Material "${materialToDelete.name}" deleted.`);
//         } catch (error) {
//             toast.dismiss(loadingToastId);
...
//             <div className="max-w-7xl mx-auto px-4 md:px-8 py-8 space-y-8 relative">
//                 <div className="bg-[#1B0A3F]/60 backdrop-blur-md rounded-xl border border-[#BF4BF6]/20 p-6 shadow-lg">
//                     <h3 className="font-bold text-white text-xl mb-4">{displayCourse.title || "Course Title Not Set"}</h3>
//                     {displayCourse.thumbnailUrl && (
//                         <img src={displayCourse.thumbnailUrl} alt={displayCourse.title} className="w-full h-60 object-cover rounded-lg mb-4 shadow-md" />
//                     )}
...
//                 onBack={handleBack}
//                 onPublish={handlePublish}
//                 disabled={isPublishing || isLoading} // also disable if loading
//             />
