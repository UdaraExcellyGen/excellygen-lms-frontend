...
import { Medal, Crown, Star, LucideProps } from 'lucide-react';
import {Learner} from "../types/leaderboard";

interface TopLearnersCardProps {
    rank: number;
    learner: Learner | undefined | null;
}
type IconType = ForwardRefExoticComponent<Omit<LucideProps, "ref"> & RefAttributes<SVGSVGElement>>;

export const TopLearnersCard: React.FC<TopLearnersCardProps> = ({ rank, learner }) => {
    if (!learner) return null;

    const backgrounds: { [key: number]: string } = {
        1: 'bg-gradient-to-br from-[#FFD700] via-[#FFA500] to-[#FF8C00]', 
        2: 'bg-gradient-to-br from-[#C0C0C0] via-[#A8A8A8] to-[#808080]', 
        3: 'bg-gradient-to-br from-[#CD7F32] via-[#B87333] to-[#A66A2E]'  
    };
    ...
    const icons: { [key: number]: IconType } = { 1: Crown, 2: Medal, 3: Medal };
            ...
            <div className="relative z-10">
                <div className="space-y-4">
                    <div className="flex items-center gap-4">
                        <div
                            className="h-16 w-16 rounded-xl bg-white/20 backdrop-blur-sm flex items-center justify-center text-2xl font-bold ring-4 ring-white/30">
                            {learner.avatar}
                        </div>
                        <div>
