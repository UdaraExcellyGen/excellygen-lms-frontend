...
// // //         const startTime = new Date(attemptResponse.startTime);

// // //         // --- THIS IS THE DEFINITIVE FIX ---
// // //         const totalDurationInSeconds = timeLimit * 60;
// // //         const elapsedSeconds = Math.floor((Date.now() - startTime.getTime()) / 1000);
        
// // //         // Ensure remaining time is never negative.
// // //         const actualRemainingSeconds = Math.max(0, totalDurationInSeconds - elapsedSeconds);
...
// // //         // If time is already up upon loading the page, trigger the auto-submit immediately.
...
// // //   // Robust timer effect synced with system clock
// // //   useEffect(() => {
...
// // //   }, [quizAttempt, isSubmitting, isTimeUp]); // Dependencies updated for robustness

// // //   // Effect to handle submission when time runs out
// // //   useEffect(() => {
// // //     // Check for quizAttempt to ensure it doesn't run before initialization
// // //     if (isTimeUp && quizAttempt) {
// // //       handleCompleteQuiz(true);
// // //     }
// // //   }, [isTimeUp, quizAttempt]); // Added quizAttempt dependency
...
// // //     if (!isAutoSubmit && unansweredCount > 0 && !window.confirm(`You have ${unansweredCount} unanswered questions. Are you sure you want to submit?`)) {
// // //       return;
// // //     }
    
// // //     // Only show toast if the quiz isn't already being submitted.
// // //     if (isAutoSubmit && !isSubmitting) {
...
// // //     const safeSeconds = Math.max(0, seconds); // Ensure we don't format a negative number
...
// // //         // --- THIS IS THE FIX ---
// // //         // We now ignore any previous timing. The timer always restarts on page load.
// // //         const startTime = new Date(); // Set the start time to the moment the page loads.
// // //         const actualRemainingSeconds = timeLimit * 60; // Set the timer to the full duration.
...
// // //   // Memoized submission handler to prevent stale state issues
// // //   const handleCompleteQuiz = useCallback(async (isAutoSubmit: boolean = false) => {
// // //     // Use a functional update to get the latest state inside the callback
// // //     setQuizAttempt(currentQuizAttempt => {
// // //       if (!currentQuizAttempt || isSubmitting) return currentQuizAttempt;

// // //       const unansweredCount = currentQuizAttempt.questions.filter(q => 
// // //         q && q.quizBankQuestionId && !currentQuizAttempt.selectedAnswers[q.quizBankQuestionId]
// // //       ).length;

// // //       if (!isAutoSubmit && unansweredCount > 0 && !window.confirm(`You have ${unansweredCount} unanswered questions. Are you sure you want to submit?`)) {
// // //         return currentQuizAttempt; // Abort if user cancels
// // //       }
...
// // //       completeQuizAttempt(currentQuizAttempt.attemptId)
// // //         .then(() => {
// // //           navigate(`/learner/quiz-results/${currentQuizAttempt.attemptId}`, {
// // //             state: { courseId }
// // //           });
// // //         })
// // //         .catch(error => {
...
// // //     });
// // //   }, [isSubmitting, navigate, courseId]);

// // //   // Robust timer effect synced with the system clock
// // //   useEffect(() => {
// // //     if (!quizAttempt || quizAttempt.isCompleted || isSubmitting) {
// // //       return;
// // //     }
...
// // //   }, [quizAttempt, isSubmitting, handleCompleteQuiz]);
...
// //   // State flag to signal when the timer has expired.
// //   const [isTimeUp, setIsTimeUp] = useState<boolean>(false);
...
// //   // Robust timer effect - its ONLY job is to count down and set the isTimeUp flag.
// //   useEffect(() => {
// //     if (!quizAttempt || quizAttempt.isCompleted || isSubmitting || isTimeUp) {
// //       return;
// //     }
...
// //       if (newTimeRemaining <= 0) {
// //         setQuizAttempt(prev => prev ? { ...prev, timeRemaining: 0 } : null);
// //         setIsTimeUp(true); // Signal that time is up
// //         clearInterval(timer);
// //       } else {
// //         setQuizAttempt(prev => prev ? { ...prev, timeRemaining: newTimeRemaining } : null);
// //       }
...
// //   // This effect safely calls the submission handler when the time is up.
// //   useEffect(() => {
// //     if (isTimeUp) {
// //       handleCompleteQuiz(true);
// //     }
...
const TakeQuiz: React.FC = () => {
  ...
  useEffect(() => {
    const initializeQuiz = async () => {
      ...
      try {
        ...
        /**
         * [NECESSARY GUARD]
         * This block is the component's own defense against re-attempts.
         * If the `startQuizAttempt` service returns an attempt that is already
         * marked as completed, this logic immediately redirects the user
         * to their results page and stops the quiz from loading. This ensures
         * that even if a user finds a way to navigate here directly, they cannot
         * start a new attempt.
         */
        if (attemptResponse.isCompleted) {
          toast.error('You have already completed this quiz');
  ...
  const handleExit = () => {
    if (quizAttempt && !quizAttempt.isCompleted) {
      if (window.confirm('Your progress will be saved. You can resume this quiz later. Are you sure you want to exit?')) {
        navigate(courseId ? `/learner/course-view/${courseId}` : -1);
      }
    } else {
      navigate(courseId ? `/learner/course-view/${courseId}` : -1);
    }
