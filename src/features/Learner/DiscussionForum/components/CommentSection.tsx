...
import * as forumApi from '../../../../api/forumApi'; // Adjust path
...
} from '../types/dto'; // Adjust path
import { useAuth } from '../../../../contexts/AuthContext'; // Adjust path
import { toast } from 'react-hot-toast';
import { formatDistanceToNow, parseISO } from 'date-fns';
import DeleteItemDialog from './DeleteItemDialog'; // Adjust path

// --- Reusable Edit Modal Component --- 
// (Consider moving to its own file: ./EditItemModal.tsx)
...
const EditItemModal: React.FC<EditItemModalProps> = ({ 
    isOpen, onClose, initialContent, onSubmit, title, isSubmitting: parentIsSubmitting 
}) => {
            ...
            <div className="bg-white p-6 rounded-lg shadow-xl w-full max-w-md">
                ...
                <form onSubmit={handleSubmit}>
                    <textarea 
                        ...
                        required disabled={actualIsSubmitting} rows={5}
                    />
...
// Extend DTO with UI-specific state for comments
interface CommentUIState extends ThreadCommentDto {
    showReplies: boolean;
    isLoadingReplies: boolean;
    isPostingReply: boolean;
    replyError: string | null;
    replies: ThreadReplyDto[]; // Replies are always part of the comment state here
}

const CommentSection: React.FC<CommentSectionProps> = ({ threadId, currentUserId }) => {
    const { user, logout: triggerAuthContextLogout } = useAuth(); // Get user details and logout function
    ...
    // State for Modals
    const [isActionLoading, setIsActionLoading] = useState(false); // General loading for edit/delete actions
    ...
    // Store the parent comment when acting on a reply for context
        ...
        try { return await apiCall(); }
            ...
            if (forumApi.isAxiosError(err) && err.response?.status === 401 && !markAsRetried) {
                try {
                    ...
                    await attemptTokenRefresh(); // From authApi.ts
    ...
    }, [triggerAuthContextLogout]); // Ensure triggerAuthContextLogout is stable or memoized in AuthContext
    ...
    const fetchComments = useCallback(async () => {
        ...
        } catch (err: any) { setErrorComments(`Failed to load comments: ${err.message}`); toast.error(`Comments Error: ${err.message}`); } 
    ...
    const handlePostComment = async () => {
        ...
        try {
            ...
            if (newCommentData) {
                ...
                // Optionally update parent thread's comment count if needed (or rely on next thread fetch)
            }
    ...
    const toggleShowReplies = (commentId: number) => {
        setComments(prev => prev.map(c => {
            if (c.id === commentId) {
                ...
                // Fetch only if opening, there are replies to fetch, they aren't already loaded, and not currently loading
    ...
    const handleEditCommentSubmit = async (newContent: string) => {
        ...
        try {
            ...
            if (updatedCommentData) {
                ...
                setEditingComment(null); // Close modal on success
            }
    ...
    const handleDeleteCommentConfirm = async () => {
        ...
        try {
            ...
            setDeletingComment(null); // Close modal on success
    ...
    const handleEditReplySubmit = async (newContent: string) => {
        ...
        try {
            ...
            if (updatedReplyData) {
                ...
                setEditingReply(null); setParentCommentForReplyAction(null); // Close modal
            }
    ...
    const handleDeleteReplyConfirm = async () => {
        ...
        try {
            ...
            setDeletingReply(null); setParentCommentForReplyAction(null); // Close modal
    ...
    return (
        <div className="space-y-5">
            ...
            {!currentUserId && (
                 <p className="text-center text-sm text-white/70 font-nunito py-3 italic">You must be logged in to comment.</p>
            )}
            ...
            <div className="space-y-4">
                {comments.map(comment => (
                    <div key={comment.id} className="bg-white/60 backdrop-blur-sm rounded-lg p-3 sm:p-4 space-y-1.5 shadow-sm relative">
                        {/* Dim overlay if action on this comment/reply is happening */}
                        ...
                        {comment.showReplies && comment.replies.length > 0 && (
                         ...
                         {comment.showReplies && comment.replies.length === 0 && !comment.isLoadingReplies && !comment.replyError && comment.repliesCount > 0 && (
                             <p className="ml-12 text-xs text-gray-500 italic mt-1">No replies to show for this comment.</p>
                         )}
                    </div>
                ))}
            </div>
