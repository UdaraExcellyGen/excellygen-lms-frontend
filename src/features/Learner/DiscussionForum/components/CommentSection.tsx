...
interface EditItemModalProps {
    ...
    title: string; 
    isSubmitting?: boolean; // Passed from parent to show loading state
}
...
const EditItemModal: React.FC<EditItemModalProps> = ({ 
    ...
    const handleSubmit = async (e: FormEvent) => {
        ...
        try { 
            await onSubmit(content); // Parent handles actual API call and potential error toasts
            // Parent will typically call onClose from its onSubmit handler if API call succeeds
        } 
        catch (err) { 
            // If onSubmit in parent re-throws, this can catch, but usually parent handles toast
    ...
    return (
        <div className="fixed inset-0 bg-[#1B0A3F]/60 backdrop-blur-sm z-[70] flex items-center justify-center p-4">
            <div className="bg-white w-full max-w-md rounded-xl border border-red-400/50 p-6 shadow-xl">
                ...
                <div className="flex justify-end gap-3"><button onClick={onClose} disabled={isDeleting} className="px-4 py-2 text-sm bg-gray-200 text-gray-700 rounded-lg hover:bg-gray-300 font-nunito">Cancel</button><button onClick={handleConfirmClick} disabled={isDeleting} className="px-4 py-2 text-sm bg-red-600 text-white rounded-lg hover:bg-red-700 font-nunito disabled:opacity-70">{isDeleting ? <RefreshCw className="animate-spin h-5 w-5 inline" /> : `Yes, Delete ${itemName}`}</button></div>
            </div>
        </div>
    );
};
// ----- End Helper Modal Components -----
...
interface CommentUIState extends ThreadCommentDto {
    showReplies: boolean; isLoadingReplies: boolean; isPostingReply: boolean; replyError: string | null;
    replies: ThreadReplyDto[];
}

const CommentSection: React.FC<CommentSectionProps> = ({ threadId, currentUserId }) => {
    const { user } = useAuth(); // No triggerAuthContextLogout as apiClient handles it
    ...
    const [isActionLoading, setIsActionLoading] = useState(false); // For edit/delete modals
    ...
    // No local callApiWithRetry needed if global apiClient from forumApi.ts handles it
    ...
    const handleEditCommentSubmit = async (newContent: string) => { 
        if (!editingComment) return; setIsActionLoading(true);
        try { 
    ...
    const handleDeleteCommentConfirm = async () => { 
        if (!deletingComment) return; setIsActionLoading(true);
        try { 
    ...
    const handleEditReplySubmit = async (newContent: string) => { 
        if (!editingReply || !parentCommentForReplyAction) return; setIsActionLoading(true);
        try { 
    ...
    const handleDeleteReplyConfirm = async () => { 
        if (!deletingReply || !parentCommentForReplyAction) return; setIsActionLoading(true);
        try { 
    ...
    const formatRelativeTime = (dateStringISO?: string): string => { 
        ...
        catch { return "just now"; }  // Fallback for potential invalid date string
    };
    ...
    return (
        <div className="space-y-5">
            {/* Add Comment Form - Ensure YOUR styling is applied */}
            {currentUserId && (
            ...
            {/* Comments List - Ensure YOUR styling */}
                ...
                {comments.map(comment => (
                    <div key={comment.id} className="bg-white/60 backdrop-blur-sm rounded-lg p-3 sm:p-4 space-y-1.5 shadow-sm relative">
                        ...
                        <div className="flex gap-3">
                             {/* Comment Avatar & Content - YOUR STYLING */}
                            ...
                            <div className="flex-1">
                                ...
                                {currentUserId && (
                                    <button onClick={() => { setReplyingToCommentId(prev => prev === comment.id ? null : comment.id); setNewReplyText('');}} className="mt-1.5 text-xs sm:text-sm text-[#9030b7] hover:text-[#6a009d] font-semibold disabled:opacity-50" disabled={isActionLoading || comment.isPostingReply}>
                        ...
                        {replyingToCommentId === comment.id && currentUserId && (
                            <div className="ml-10 sm:ml-12 mt-2 pt-2 pl-2 sm:pl-3 border-l-2 border-[#D0A0E6]/70">
                                <textarea value={newReplyText} onChange={(e) => setNewReplyText(e.target.value)} placeholder={`Write a reply...`} 
                        ...
                        {/* Replies Section - YOUR STYLING */}
                        {comment.repliesCount > 0 && (
                             <button onClick={() => toggleShowReplies(comment.id)} className="text-xs font-semibold text-[#6a009d] hover:underline mt-2 ml-10 sm:ml-12 flex items-center disabled:opacity-50" disabled={comment.isLoadingReplies || isActionLoading}>
                        ...
                        {comment.showReplies && comment.replies.length > 0 && (
                            <div className="ml-10 sm:ml-12 mt-2 space-y-2 pt-1 pl-2 sm:pl-3 border-l-2 border-dashed border-[#D0A0E6]/50">
                                {comment.replies.map(reply => (
                                    <div key={reply.id} className="flex gap-2"> {/* YOUR reply item styling */}
                         ...
                         {comment.showReplies && comment.replies.length === 0 && !comment.isLoadingReplies && !comment.replyError && comment.repliesCount > 0 && (
                             <p className="ml-12 text-xs text-gray-500 italic mt-1">No replies to show.</p>
                         )}
                    </div>
                ))}
            </div>
