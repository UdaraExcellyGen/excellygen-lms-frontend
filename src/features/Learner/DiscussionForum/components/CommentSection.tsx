        ...
        <div className="fixed inset-0 bg-black/30 backdrop-blur-sm z-[60] flex items-center justify-center p-4">
            <div className="bg-white p-6 rounded-lg shadow-xl w-full max-w-md">
                ...
                <form onSubmit={handleSubmit}>
                    <textarea value={content} onChange={(e) => setContent(e.target.value)} className="w-full p-2 border rounded min-h-[100px] text-sm font-nunito text-[#1B0A3F] border-[#BF4BF6]/50 focus:ring-1 focus:ring-[#BF4BF6]" required disabled={actualIsSubmitting} rows={5} />
                    <div className="mt-4 flex justify-end gap-2">
                        ...
                        <button type="submit" disabled={actualIsSubmitting || !content.trim()} className="px-3 py-1.5 text-sm bg-[#7A00B8] text-white rounded hover:bg-[#5f0090] disabled:opacity-60 font-nunito">{actualIsSubmitting ? <RefreshCw className="animate-spin h-4 w-4 inline"/> : 'Save Changes'}</button>
                    </div>
                </form>
            </div>
        </div>
...
const CommentSection: React.FC<CommentSectionProps> = ({ threadId, currentUserId, onCommentPosted }) => {
    ...
    const handlePostComment = async () => { 
        ...
        try {
            ...
            setNewCommentText(''); toast.success("Comment posted!"); onCommentPosted();
    ...
    const handlePostReply = async (commentId: number) => { 
        ...
        try {
            ...
            setNewReplyText(''); setReplyingToCommentId(null); toast.success("Reply posted!"); onCommentPosted();
    ...
    return (
        <div className="space-y-5">
            {currentUserId && (
                <div className="flex gap-3 sm:gap-4">
                    ...
                    <div className="flex-1">
                        <textarea value={newCommentText} onChange={(e) => setNewCommentText(e.target.value)} placeholder="Add a comment..." className="w-full bg-[#FDF6FF]/70 border border-[#D0A0E6]/50 rounded-lg px-3 py-2 text-[#1B0A3F] focus:ring-1 focus:ring-[#BF4BF6] focus:border-transparent font-nunito text-sm sm:text-base min-h-[70px]" disabled={isPostingComment || isActionLoading} />
                        <button onClick={handlePostComment} disabled={isPostingComment || isActionLoading || !newCommentText.trim()} className="mt-2 px-4 py-1.5 text-sm bg-gradient-to-r from-[#BF4BF6] to-[#7A00B8] text-white rounded-lg hover:from-[#D68BF9] hover:to-[#BF4BF6] font-nunito disabled:opacity-60 flex items-center justify-center w-full sm:w-auto min-w-[150px]">
            ...
            {!currentUserId && <p className="text-center text-sm text-white/70 font-nunito py-3 italic">You must be logged in to comment.</p>}
            {comments.length === 0 && !isLoadingComments && <p className="text-center text-sm text-white/50 font-nunito py-3">No comments yet.</p>}
                ...
                {comments.map(comment => (
                    <div key={comment.id} className="bg-white/60 backdrop-blur-sm rounded-lg p-3 sm:p-4 space-y-1.5 shadow-sm relative">
                        {((editingComment?.id === comment.id || deletingComment?.id === comment.id || parentCommentForReplyAction?.id === comment.id) && isActionLoading) && (
                            <div className="absolute inset-0 bg-white/50 z-10 flex items-center justify-center rounded-lg"><RefreshCw className="h-5 w-5 animate-spin text-[#7A00B8]"/></div>
                        )}
                                ...
                                <div className="flex items-center justify-between">
                                    ...
                                    {comment.isCurrentUserAuthor && currentUserId === comment.author?.id && (
                                        <div className="flex items-center gap-1 flex-shrink-0">
                                            <button onClick={() => setEditingComment(comment)} title="Edit comment" className="p-1 text-xs text-blue-600 hover:text-blue-800 disabled:opacity-50" disabled={isActionLoading}><Edit2 size={14}/></button>
                                            <button onClick={() => setDeletingComment(comment)} title="Delete comment" className="p-1 text-xs text-red-500 hover:text-red-700 disabled:opacity-50" disabled={isActionLoading}><Trash2 size={14}/></button>
                                        </div>
                                    )}
                                </div>
                                {/* --- MODIFIED COMMENT CONTENT RENDERER --- */}
                                <MarkdownRenderer content={comment.content} className="prose prose-sm max-w-none mt-1 text-[#4a0c75] break-words" />
                                {currentUserId && (<button onClick={() => { setReplyingToCommentId(prev => prev === comment.id ? null : comment.id); setNewReplyText(''); }} className="mt-1.5 text-xs sm:text-sm text-[#9030b7] hover:text-[#6a009d] font-semibold disabled:opacity-50" disabled={isActionLoading || comment.isPostingReply}>{replyingToCommentId === comment.id ? 'Cancel' : 'Reply'}</button>)}
                            </div>
                        </div>
                        {replyingToCommentId === comment.id && currentUserId && (
                            <div className="ml-10 sm:ml-12 mt-2 pt-2 pl-2 sm:pl-3 border-l-2 border-[#D0A0E6]/70">
                                <textarea value={newReplyText} onChange={(e) => setNewReplyText(e.target.value)} placeholder="Write a reply..." className="w-full bg-white/70 border border-[#CBB0DB]/60 rounded-md px-2.5 py-1.5 text-[#1B0A3F] focus:ring-1 focus:ring-[#BF4BF6] text-xs sm:text-sm min-h-[50px]" disabled={comment.isPostingReply || isActionLoading} />
                                <div className="mt-1.5 flex gap-2">
                                    <button onClick={() => handlePostReply(comment.id)} disabled={comment.isPostingReply || isActionLoading || !newReplyText.trim()} className="px-3 py-1 text-xs sm:text-sm bg-gradient-to-r from-[#BF4BF6]/80 to-[#7A00B8]/80 text-white rounded hover:from-[#D68BF9]/80 hover:to-[#BF4BF6]/80 font-nunito disabled:opacity-60 flex items-center justify-center min-w-[80px]">
                                        {comment.isPostingReply ? (<RefreshCw className="h-3 w-3 animate-spin" />) : (<><Send className="h-3 w-3" /><span className="ml-1.5">Post</span></>)}
                                    </button>
                                    <button onClick={() => setReplyingToCommentId(null)} disabled={comment.isPostingReply || isActionLoading} className="px-3 py-1 text-xs sm:text-sm bg-gray-100 text-[#52007C] rounded hover:bg-gray-200 font-nunito">Cancel</button>
                                </div>
                            </div>
                        )}
                        {comment.repliesCount > 0 && (<button onClick={() => toggleShowReplies(comment.id)} className="text-xs font-semibold text-[#6a009d] hover:underline mt-2 ml-10 sm:ml-12 flex items-center disabled:opacity-50" disabled={comment.isLoadingReplies || isActionLoading}><MessageCircle className={`h-3 w-3 mr-1 ${comment.showReplies ? 'fill-[#7A00B8]/70 text-white' : 'text-[#7A00B8]'}`} />{comment.showReplies ? 'Hide' : `View ${comment.repliesCount}`} Reply{comment.repliesCount === 1 ? '' : 'ies'}{comment.isLoadingReplies && <RefreshCw className="h-3 w-3 animate-spin inline ml-1.5" />}</button>)}
                        {comment.replyError && comment.showReplies && !comment.isLoadingReplies && (<p className="ml-12 mt-1 text-xs text-red-600 bg-red-50 p-1.5 rounded ">Error: {comment.replyError} <span onClick={() => fetchRepliesForComment(comment.id)} className="underline cursor-pointer font-medium">Retry</span></p>)}
                                            ...
                                            <div className="flex items-center justify-between">
                                                ...
                                                {reply.isCurrentUserAuthor && currentUserId === reply.author?.id && (
                                                    <div className="flex items-center gap-0.5 flex-shrink-0">
                                                        <button onClick={() => { setParentCommentForReplyAction(comment); setEditingReply(reply); }} title="Edit reply" className="p-0.5 text-[10px] text-blue-600 hover:text-blue-800 disabled:opacity-50" disabled={isActionLoading || comment.isPostingReply}><Edit2 size={12}/></button>
                                                        <button onClick={() => { setParentCommentForReplyAction(comment); setDeletingReply(reply); }} title="Delete reply" className="p-0.5 text-[10px] text-red-500 hover:text-red-700 disabled:opacity-50" disabled={isActionLoading || comment.isPostingReply}><Trash2 size={12}/></button>
                                                    </div>
                                                )}
                                            </div>
                                            {/* --- MODIFIED REPLY CONTENT RENDERER --- */}
                        ...
                        {comment.showReplies && comment.replies.length === 0 && !comment.isLoadingReplies && !comment.replyError && comment.repliesCount > 0 && (<p className="ml-12 text-xs text-gray-500 italic mt-1">No replies to show.</p>)}
                    </div>
                ))}
            </div>
            {editingComment && !editingReply && (<EditItemModal isOpen={!!editingComment} onClose={() => setEditingComment(null)} initialContent={editingComment.content} onSubmit={handleEditCommentSubmit} title="Edit Comment" isSubmitting={isActionLoading} />)}
            {deletingComment && (<DeleteItemDialog isOpen={!!deletingComment} onClose={() => setDeletingComment(null)} onConfirm={handleDeleteCommentConfirm} itemName="comment" itemContentPreview={deletingComment.content} />)}
            {editingReply && parentCommentForReplyAction && (<EditItemModal isOpen={!!editingReply} onClose={() => { setEditingReply(null); setParentCommentForReplyAction(null); }} initialContent={editingReply.content} onSubmit={handleEditReplySubmit} title="Edit Reply" isSubmitting={isActionLoading} />)}
            {deletingReply && parentCommentForReplyAction && (<DeleteItemDialog isOpen={!!deletingReply} onClose={() => { setDeletingReply(null); setParentCommentForReplyAction(null);}} onConfirm={handleDeleteReplyConfirm} itemName="reply" itemContentPreview={deletingReply.content} />)}
        </div>
    );
};
