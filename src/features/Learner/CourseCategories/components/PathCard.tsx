    ...
    <div className="group bg-white/10 backdrop-blur-lg rounded-xl p-6 relative min-h-[400px] transition-all duration-300">
      ...
      <div className="space-y-3 mb-16">
        ...
        <div className="flex items-center gap-2 text-gray-300">
          ...
          <span>{path.activeUsers.toLocaleString()} Active Learners</span> {/* DYNAMIC: Display activeUsers */}
        </div>
        <div className="flex items-center gap-2 text-gray-300">
          <Clock className="h-4 w-4" />
          <span>{path.avgDuration}</span> {/* DYNAMIC: Display avgDuration */}
        </div>
      </div>

      <button
        onClick={() => onExplore(path.id)} // Pass path.id (category ID) for navigation
        className="absolute bottom-6 left-6 right-6 px-6 py-3 bg-gradient-to-r from-[#BF4BF6] to-[#7A00B8] text-white rounded-lg
                   hover:from-[#D68BF9] hover:to-[#BF4BF6] transition-all duration-300 font-nunito shadow-lg disabled:opacity-50"
      >
        Explore Path
      </button>
    </div>
